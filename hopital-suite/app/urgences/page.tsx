...
const PATIENTS = [
  { patient: "Modou Diagne", motif: "Douleur thoracique", priorite: "P1", attente: "0 min", box: "Box 1" },
  { patient: "Aïda Fall", motif: "Fracture ouverte tibia", priorite: "P1", attente: "5 min", box: "Box 2" },
  { patient: "Cheikh Sarr", motif: "Fièvre 39.8°C enfant", priorite: "P2", attente: "22 min", box: "Box 5" },
  { patient: "Bineta Diouf", motif: "Entorse cheville", priorite: "P3", attente: "48 min", box: "Salle d'attente" },
  { patient: "Ousmane Ka", motif: "Céphalées persistantes", priorite: "P3", attente: "55 min", box: "Salle d'attente" },
  { patient: "Rama Sylla", motif: "Renouvellement ordonnance", priorite: "P4", attente: "1h10", box: "Salle d'attente" },
]

const prioriteCouleur: Record<string, string> = { P1: "#ef4444", P2: "#f59e0b", P3: "#0ea5e9", P4: "#64748b" }
    ...
    <>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "3rem 1.5rem" }}>
        ...
        <div className="au1" style={{ display: "flex", alignItems: "flex-start", gap: 20, marginBottom: "3rem" }}>
          ...
          <div>
            ...
            <h1 style={{ fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: 900, lineHeight: 1.1, margin: "10px 0" }}>
              <span style={{ background: `linear-gradient(135deg, #fff, ${COUL})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Urgences SAU</span>
            </h1>
            <p style={{ fontSize: "clamp(0.95rem, 2vw, 1.15rem)", color: "rgba(255,255,255,0.55)", lineHeight: 1.7, maxWidth: 600 }}>
              Pilotage en temps réel du Service d&apos;Accueil des Urgences : occupation des box, priorités et temps d&apos;attente.
            </p>
          </div>
        </div>
        ...
        <div className="au2" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: "3rem" }}>
          {[
            { val: "23", label: "Patients en Attente" },
            { val: "38 min", label: "Temps d'Attente Moyen" },
            { val: "14/18", label: "Box Occupés" },
            { val: "3", label: "Cas Critiques (P1)" },
          ].map((s, i) => (
        ...
        <div className="au3" style={{ marginBottom: "3rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 260px), 1fr))", gap: 12 }}>
            {[
              { icon: "🛏️", titre: "Vue des Box en Temps Réel", desc: "Occupation et disponibilité de chaque box" },
              { icon: "🚦", titre: "Classification P1 à P4", desc: "Priorisation selon la gravité clinique" },
              { icon: "📢", titre: "Alerte de Saturation", desc: "Notification dès dépassement de seuil critique" },
              { icon: "🔗", titre: "Liaison Triage IA & AmbuTrack", desc: "Continuité entre orientation et arrivée" },
            ].map((f, i) => (
        ...
        <div className="au3" style={{ marginBottom: "3rem" }}>
          <h2 style={{ fontSize: "clamp(1.3rem, 2.5vw, 1.8rem)", fontWeight: 800, marginBottom: "1.5rem" }}>File d&apos;Attente — En Direct</h2>
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, overflow: "hidden" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {["Patient", "Motif", "Priorité", "Attente", "Box"].map(h => (
                ...
                <tbody>
                  {PATIENTS.map((p, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                      <td style={{ padding: "10px 14px", fontWeight: 600, color: "#fff" }}>{p.patient}</td>
                      <td style={{ padding: "10px 14px", color: "rgba(255,255,255,0.6)" }}>{p.motif}</td>
                      <td style={{ padding: "10px 14px" }}>
                        <span style={{ background: `${prioriteCouleur[p.priorite]}22`, color: prioriteCouleur[p.priorite], padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700 }}>{p.priorite}</span>
                      </td>
                      <td style={{ padding: "10px 14px", color: "rgba(255,255,255,0.6)" }}>{p.attente}</td>
                      <td style={{ padding: "10px 14px", color: "rgba(255,255,255,0.6)" }}>{p.box}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="au4" style={{ background: `${COUL}10`, border: `1px solid ${COUL}25`, borderRadius: 16, padding: "2rem", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: "1.5rem" }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.5)" }}>Contactez Processingenierie pour déployer le module Urgences SAU dans votre infrastructure.</p>
          </div>
