...
const SUIVIS = [
  { patient: "Ibrahima Diallo", proche: "Mariama Diallo (épouse)", statut: "En chirurgie", maj: "Il y a 12 min" },
  { patient: "Fatou Ndiaye", proche: "Cheikh Ndiaye (fils)", statut: "En salle de réveil", maj: "Il y a 35 min" },
  { patient: "Rokhaya Sy", proche: "Awa Sy (mère)", statut: "En maternité — stable", maj: "Il y a 1h" },
  { patient: "Modou Gueye", proche: "Astou Gueye (fille)", statut: "Sorti — retour prévu demain", maj: "Il y a 2h" },
]

const statutCouleur: Record<string, string> = { "En chirurgie": "#f59e0b", "En salle de réveil": "#0ea5e9", "En maternité — stable": "#22c55e", "Sorti — retour prévu demain": "#64748b" }
    ...
    <>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "3rem 1.5rem" }}>
        ...
        <div className="au1" style={{ display: "flex", alignItems: "flex-start", gap: 20, marginBottom: "3rem" }}>
          ...
          <div>
            ...
            <h1 style={{ fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: 900, lineHeight: 1.1, margin: "10px 0" }}>
              <span style={{ background: `linear-gradient(135deg, #fff, ${COUL})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Proches-Aidants</span>
            </h1>
            <p style={{ fontSize: "clamp(0.95rem, 2vw, 1.15rem)", color: "rgba(255,255,255,0.55)", lineHeight: 1.7, maxWidth: 600 }}>
              Suivi à distance pour les familles : statut d&apos;hospitalisation, messagerie et créneaux de visite.
            </p>
          </div>
        </div>
        ...
        <div className="au2" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: "3rem" }}>
          {[
            { val: "342", label: "Familles Connectées" },
            { val: "189", label: "Messages Échangés (Jour)" },
            { val: "156", label: "Patients Suivis" },
            { val: "94%", label: "Satisfaction des Familles" },
          ].map((s, i) => (
        ...
        <div className="au3" style={{ marginBottom: "3rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 260px), 1fr))", gap: 12 }}>
            {[
              { icon: "📡", titre: "Statut en Direct", desc: "Étapes clés de l'hospitalisation (bloc, réveil, sortie)" },
              { icon: "💬", titre: "Messagerie Sécurisée", desc: "Échange avec le service en charge du patient" },
              { icon: "📅", titre: "Créneaux de Visite", desc: "Réservation en ligne des horaires de visite" },
              { icon: "🔔", titre: "Notifications d'Étapes Clés", desc: "Alerte automatique aux moments importants" },
            ].map((f, i) => (
        ...
        <div className="au3" style={{ marginBottom: "3rem" }}>
          <h2 style={{ fontSize: "clamp(1.3rem, 2.5vw, 1.8rem)", fontWeight: 800, marginBottom: "1.5rem" }}>Suivis Actifs</h2>
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, overflow: "hidden" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {["Patient", "Proche référent", "Statut", "Dernière mise à jour"].map(h => (
                ...
                <tbody>
                  {SUIVIS.map((s, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                      <td style={{ padding: "10px 14px", fontWeight: 600, color: "#fff" }}>{s.patient}</td>
                      <td style={{ padding: "10px 14px", color: "rgba(255,255,255,0.6)" }}>{s.proche}</td>
                      ...
                      <td style={{ padding: "10px 14px", color: "rgba(255,255,255,0.45)" }}>{s.maj}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="au4" style={{ background: `${COUL}10`, border: `1px solid ${COUL}25`, borderRadius: 16, padding: "2rem", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: "1.5rem" }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.5)" }}>Contactez Processingenierie pour déployer le module Proches-Aidants dans votre infrastructure.</p>
          </div>
