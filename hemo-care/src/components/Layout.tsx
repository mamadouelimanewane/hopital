...
import {
  LayoutDashboard, Users, CalendarClock, Cpu, LineChart, Settings, Bell, Search,
  Menu, Droplet, X, ChevronRight, Wifi, WifiOff, LogOut, Shield,
...
const navigation = [
  ...
  { name: 'Planning',        href: '/planning',       icon: CalendarClock,   badge: null },
  { name: 'Générateurs',     href: '/generateurs',    icon: Cpu,             badge: null },
  { name: 'Suivi Poids',     href: '/suivi-poids',    icon: LineChart,       badge: null },
];
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-5"
          ...
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg bg-sky-500/10">
              <Droplet size={22} className="text-sky-500" />
            </div>
            <div>
              <span className="text-sm font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>Hemo-Care</span>
              <div className="text-[10px] leading-none" style={{ color: 'var(--text-muted)' }}>Centre d'Hémodialyse</div>
            </div>
          </div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon size={18} className="flex-shrink-0 transition-colors"
                    style={{ color: isActive ? '#0ea5e9' : 'var(--text-faint)' }} />
                  {item.name}
                </div>
          ...
          <Link
            ...
            style={location.pathname === '/settings' ? { background: 'rgba(14,165,233,0.12)', color: '#0ea5e9' } : { color: 'var(--text-muted)' }}
        ...
        <header className="h-16 sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 backdrop-blur-xl"
          ...
          <div className="flex-1 max-w-md mx-4 hidden md:block">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Rechercher un patient, une séance..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full rounded-xl py-2 pl-9 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/40 transition-all"
                style={{
              ...
              {notifOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-80 rounded-2xl shadow-2xl z-50 overflow-hidden"
                  ...
                  <div className="flex items-center justify-between px-4 py-3"
                    ...
                    <button onClick={markAllRead} className="text-[10px] text-sky-500 transition-colors">
                      Tout marquer lu
                    </button>
                  </div>
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      return (
                        <div key={n.id}
                          className={`flex gap-3 px-4 py-3 transition-colors ${!n.read ? 'bg-sky-500/5' : ''}`}
                          style={{ borderBottom: '1px solid var(--border-soft)' }}
                          onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--bg-elevated)'}
                          onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = !n.read ? 'rgba(14,165,233,0.05)' : ''}
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-1">
                              ...
                              {!n.read && <div className="w-1.5 h-1.5 rounded-full bg-sky-400 shrink-0 mt-1" />}
                            </div>
                  ...
                  <div className="px-4 py-2.5" style={{ borderTop: '1px solid var(--border-soft)' }}>
                    <button
                      ...
                      className="text-[11px] text-sky-500 hover:text-sky-400 transition-colors font-medium"
                    >
