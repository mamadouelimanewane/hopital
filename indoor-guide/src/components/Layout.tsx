...
import {
  LayoutDashboard, Map, Users, Route, ParkingSquare, Settings, Bell, Search,
  Menu, MapPinned, X, ChevronRight,
...
const navigation = [
  ...
  { name: 'Carte Interactive', href: '/carte',      icon: Map,             badge: null },
  { name: 'Visiteurs',       href: '/visiteurs',    icon: Users,           badge: null },
  { name: 'Itinéraires',     href: '/itineraires',  icon: Route,           badge: null },
  { name: 'Parking',         href: '/parking',      icon: ParkingSquare,   badge: null },
];
    ...
    <div className="min-h-screen flex font-['Inter',system-ui,sans-serif]"
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-5"
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="text-sm font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>Indoor-Guide</span>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon size={18} className="flex-shrink-0 transition-colors"
                    style={{ color: isActive ? '#3b82f6' : 'var(--text-faint)' }} />
                  {item.name}
                </div>
          ...
          <Link
            ...
            style={location.pathname === '/settings' ? { background: 'rgba(59,130,246,0.12)', color: '#3b82f6' } : { color: 'var(--text-muted)' }}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex-1 max-w-md mx-4 hidden md:block">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl py-2 pl-9 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition-all"
                style={{
              ...
              {notifOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-80 rounded-2xl shadow-2xl z-50 overflow-hidden"
                  ...
                  <div className="flex items-center justify-between px-4 py-3"
                    ...
                    <button onClick={markAllRead} className="text-[10px] text-blue-500 transition-colors">
                      Tout marquer lu
                    </button>
                  </div>
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      return (
                        <div key={n.id}
                          className={`flex gap-3 px-4 py-3 transition-colors ${!n.read ? 'bg-blue-500/5' : ''}`}
                          style={{ borderBottom: '1px solid var(--border-soft)' }}
                          onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--bg-elevated)'}
                          onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = !n.read ? 'rgba(59,130,246,0.05)' : ''}
                        >
        ...
        <main className="flex-1 overflow-auto" style={{ background: 'var(--bg-app)' }}>
          ...
          <footer className="text-center py-4 text-[10px]" style={{ color: 'var(--text-faint)' }}>
