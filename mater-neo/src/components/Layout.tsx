...
import {
  LayoutDashboard, Baby, Activity, Settings, Bell, Search,
  Menu, HeartPulse, X, ChevronRight,
  Wifi, WifiOff, LogOut, Shield, CheckCircle2, AlertTriangle, Info, XCircle,
} from 'lucide-react';
...
const navigation = [
  ...
  { name: 'Grossesses',      href: '/grossesses',    icon: Baby,            badge: null },
  { name: 'Partogramme',     href: '/partogramme',   icon: Activity,        badge: null },
  { name: 'Accouchements',   href: '/accouchements', icon: HeartPulse,      badge: null },
  { name: 'Néonatologie',    href: '/neonatologie',  icon: Baby,            badge: null },
];
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-5"
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="text-sm font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>Mater-Neo</span>
              <div className="text-[10px] leading-none" style={{ color: 'var(--text-muted)' }}>Maternité & Néonat. · Ndamatou</div>
            </div>
          </div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon size={18} className="flex-shrink-0 transition-colors"
                    style={{ color: isActive ? '#ec4899' : 'var(--text-faint)' }} />
                  {item.name}
                </div>
                {isActive && <ChevronRight size={14} className="text-pink-500" />}
              </Link>
            );
          })}
          ...
          <Link
            ...
            style={location.pathname === '/settings' ? { background: 'rgba(236,72,153,0.12)', color: '#ec4899' } : { color: 'var(--text-muted)' }}
        ...
        <header className="h-16 sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 backdrop-blur-xl"
          ...
          <div className="flex-1 max-w-md mx-4 hidden md:block">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Rechercher une patiente, un dossier..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full rounded-xl py-2 pl-9 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/40 transition-all"
                style={{
              ...
              {notifOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-80 rounded-2xl shadow-2xl z-50 overflow-hidden"
                  ...
                  <div className="flex items-center justify-between px-4 py-3"
                    ...
                    <button onClick={markAllRead} className="text-[10px] text-pink-500 transition-colors">
                      Tout marquer lu
                    </button>
                  </div>
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      return (
                        <div key={n.id}
                          className={`flex gap-3 px-4 py-3 transition-colors ${!n.read ? 'bg-pink-500/5' : ''}`}
                          style={{ borderBottom: '1px solid var(--border-soft)' }}
                          onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--bg-elevated)'}
                          onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = !n.read ? 'rgba(236,72,153,0.05)' : ''}
                        >
