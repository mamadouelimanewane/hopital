...
const STATS = [
  ...
  { val: '24/7', label: 'Suivi continu',     icon: '🛡️' },
];

const MODULES = [
  { icon: '🏷️', label: 'Traçabilité',  desc: 'Suivi par code-barres' },
  { icon: '⚖️', label: 'Pesée',        desc: 'Contrôle des volumes' },
  { icon: '🔥', label: 'Destruction',  desc: 'Incinération sécurisée' },
  { icon: '📜', label: 'Conformité',   desc: 'Normes environnementales' },
  { icon: '📦', label: 'Containers',   desc: 'Suivi des remplissages' },
  { icon: '🧾', label: 'Certificats',  desc: 'Registres d\'incinération' },
];
    ...
    <div className="min-h-screen overflow-x-hidden" style={{ background: '#060c18', color: '#fff' }}>
        ...
        <svg className="absolute bottom-0 left-0 w-full h-[50vh] opacity-[0.04]"
          ...
          <defs>
            <pattern id="grid" width="60" height="60" patternUnits="userSpaceOnUse">
              <path d="M 60 0 L 0 0 0 60" fill="none" stroke="#84cc16" strokeWidth="0.8"/>
            </pattern>
          </defs>
          ...
          {[0,45,90,135].map(a => (
            <g key={a} transform={`rotate(${a} 100 100)`}>
              ...
              <polygon points="100,35 165,68 165,132 100,165 35,132 35,68"
                fill="none" stroke="#84cc16" strokeWidth="0.6"/>
            </g>
          ))}
          <circle cx="100" cy="100" r="20" fill="none" stroke="#84cc16" strokeWidth="1"/>
          <circle cx="100" cy="100" r="8" fill="#84cc16" opacity="0.3"/>
        </svg>
      </div>

      {/* ══ NAVBAR ══════════════════════════════════════════════ */}
      <nav className="relative z-20 flex items-center justify-between px-8 py-5 border-b"
        style={{ borderColor: 'rgba(132,204,22,0.1)', backdropFilter: 'blur(10px)', background: 'rgba(6,12,24,0.7)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-black text-white text-sm tracking-wide">Waste-Control</div>
            <div className="text-[10px] tracking-[0.15em] uppercase" style={{ color: '#84cc16' }}>Touba · Sénégal</div>
          </div>
        </div>

        <div className="hidden md:flex items-center gap-2 text-[11px] font-medium text-slate-400">
          <div className="w-2 h-2 rounded-full bg-lime-400 animate-pulse" />
          Système opérationnel · Waste-Control v1.0
        </div>
      ...
      <section className="relative z-10 flex flex-col items-center justify-center text-center px-6 pt-20 pb-16"
        ...
        <h1
          ...
          style={{
            ...
            background: 'linear-gradient(135deg, #ffffff 0%, #a3e635 40%, #84cc16 70%, #ffffff 100%)',
        ...
        <div
          ...
          <div className="h-px w-20 opacity-40" style={{ background: '#84cc16' }} />
          <span className="text-sm font-bold tracking-[0.3em] uppercase" style={{ color: '#84cc16' }}>
        ...
        <p
          ...
          Traçabilité complète des déchets infectieux et hospitaliers (DASRI) à l'Hôpital Ndamatou de Touba
        </p>
        ...
        <div
          ...
          <button
            ...
            style={{
              ...
              color: '#0a1502',
              boxShadow: '0 8px 40px rgba(132,204,22,0.35)',
            }}>
            🗑️ Accéder au système
        ...
        <div
          ...
          <span className="font-black text-2xl" style={{ color: '#84cc16' }}>{counter}%</span>
          <span>de traçabilité des déchets médicaux</span>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4">
          {STATS.map((s, i) => (
            <div
              ...
              <div className="font-black text-3xl mb-1" style={{ color: '#84cc16' }}>{s.val}</div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-10">
            <div className="flex items-center justify-center gap-3 mb-3">
              <div className="h-px w-16 opacity-30" style={{ background: '#84cc16' }} />
              <span className="text-[10px] font-bold tracking-[0.25em] uppercase" style={{ color: '#84cc16' }}>
                Fonctionnalités
              </span>
              <div className="h-px w-16 opacity-30" style={{ background: '#84cc16' }} />
            </div>
            <h2 className="text-2xl font-black text-white">Une plateforme complète de gestion DASRI</h2>
            <p className="text-slate-500 text-sm mt-2">De la collecte à la destruction, une traçabilité de bout en bout</p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {MODULES.map((m, i) => (
              <div
                ...
                onMouseEnter={e => {
                  (e.currentTarget as HTMLElement).style.background = 'rgba(132,204,22,0.06)';
                  (e.currentTarget as HTMLElement).style.borderColor = 'rgba(132,204,22,0.2)';
                }}
                onMouseLeave={e => {
      ...
      <section className="relative z-10 px-6 py-12 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="text-xs font-bold tracking-widest uppercase mb-2" style={{ color: '#84cc16' }}>Normes respectées</div>
            <div className="flex flex-wrap gap-2">
              {['DASRI Conforme', 'ISO 14001', 'Norme OMS', 'Ministère Santé SN'].map(c => (
                <span key={c} className="px-3 py-1 rounded-full text-[10px] font-bold border"
                  style={{ borderColor: 'rgba(132,204,22,0.25)', color: '#84cc16', background: 'rgba(132,204,22,0.05)' }}>
                  ✓ {c}
                </span>
              ))}
            </div>
          ...
          <button
            ...
            style={{
              background: 'linear-gradient(135deg, #84cc16, #a3e635)',
              color: '#0a1502',
              boxShadow: '0 4px 24px rgba(132,204,22,0.25)',
            }}>
            → Accéder au système
          </button>
        </div>
