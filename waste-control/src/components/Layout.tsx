...
import {
  LayoutDashboard, Trash2, Package, Flame, ShieldCheck, Settings, Bell, Search,
  Menu, Recycle, X, ChevronRight, Wifi, WifiOff, LogOut, Shield,
...
const navigation = [
  ...
  { name: 'Collectes',       href: '/collectes',    icon: Trash2,          badge: null },
  { name: 'Containers',      href: '/containers',   icon: Package,         badge: '2'  },
  { name: 'Destruction',     href: '/destruction',  icon: Flame,           badge: null },
  { name: 'Conformité',      href: '/conformite',   icon: ShieldCheck,     badge: null },
];
    ...
    <div className="min-h-screen flex font-['Inter',system-ui,sans-serif]"
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-5"
          ...
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg bg-lime-500/10">
              <Recycle size={22} className="text-lime-500" />
            </div>
            <div>
              <span className="text-sm font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>Waste-Control</span>
              <div className="text-[10px] leading-none" style={{ color: 'var(--text-muted)' }}>v1.0 · Ndamatou</div>
            </div>
          </div>
        ...
        <div className={cn(
          ...
          online ? "bg-lime-500/10 text-lime-500" : "bg-amber-500/10 text-amber-500"
        )}>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon size={18} className="flex-shrink-0 transition-colors"
                    style={{ color: isActive ? '#84cc16' : 'var(--text-faint)' }} />
                  {item.name}
                </div>
                {item.badge && (
                  <span className="text-[10px] font-bold rounded-full px-1.5 py-0.5 min-w-[18px] text-center bg-rose-500 text-white">
                    {item.badge}
                  </span>
                )}
                {isActive && !item.badge && <ChevronRight size={14} className="text-lime-500" />}
              </Link>
            );
          })}
          ...
          <Link
            ...
            style={location.pathname === '/settings' ? { background: 'rgba(132,204,22,0.12)', color: '#84cc16' } : { color: 'var(--text-muted)' }}
        ...
        <header className="h-16 sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 backdrop-blur-xl"
          ...
          <div className="flex-1 max-w-md mx-4 hidden md:block">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Rechercher un code-barres, une zone..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full rounded-xl py-2 pl-9 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-lime-500/40 transition-all"
                style={{
              ...
              {notifOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-80 rounded-2xl shadow-2xl z-50 overflow-hidden"
                  ...
                  <div className="flex items-center justify-between px-4 py-3"
                    ...
                    <button onClick={markAllRead} className="text-[10px] text-lime-500 transition-colors">
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      return (
                        <div key={n.id}
                          className={`flex gap-3 px-4 py-3 transition-colors ${!n.read ? 'bg-lime-500/5' : ''}`}
                          style={{ borderBottom: '1px solid var(--border-soft)' }}
                          onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--bg-elevated)'}
                          onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = !n.read ? 'rgba(132,204,22,0.05)' : ''}
                        >
                  ...
                  <div className="px-4 py-2.5" style={{ borderTop: '1px solid var(--border-soft)' }}>
                    <button
                      ...
                      className="text-[11px] text-lime-500 hover:text-lime-400 transition-colors font-medium"
                    >
