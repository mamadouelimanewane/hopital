...
import {
  LayoutDashboard, Snowflake, FileText, Users, Settings, Bell, Search,
  Menu, X, ChevronRight, Wifi, WifiOff, LogOut, Shield,
  CheckCircle2, AlertTriangle, Info, XCircle, BookOpen
} from 'lucide-react';
...
const navigation = [
  ...
  { name: 'Registre des défunts', href: '/registre', icon: BookOpen },
  { name: 'Casiers', href: '/casiers', icon: Snowflake },
  { name: 'Démarches', href: '/demarches', icon: FileText },
  { name: 'Familles', href: '/familles', icon: Users },
];
...
const notifTypeColor = { success: 'text-emerald-400', warning: 'text-amber-400', error: 'text-rose-400', info: 'text-blue-400' };
...
export default function Layout() {
  ...
  const currentPage = navigation.find(n => n.href === location.pathname)?.name || 'Tableau de bord';
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-5"
          ...
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg bg-slate-500/10 text-lg leading-none">
              🕊️
            </div>
            <div>
              <span className="text-sm font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>Morgue-Sync</span>
              <div className="text-[10px] leading-none" style={{ color: 'var(--text-muted)' }}>Gestion Funéraire · Ndamatou</div>
            </div>
          </div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon size={18} className="flex-shrink-0 transition-colors"
                    style={{ color: isActive ? '#cbd5e1' : 'var(--text-faint)' }} />
                  {item.name}
                </div>
                {isActive && <ChevronRight size={14} style={{ color: '#cbd5e1' }} />}
              </Link>
            );
          })}
          ...
          <Link
            ...
            style={location.pathname === '/settings' ? { background: 'rgba(148,163,184,0.14)', color: '#e2e8f0' } : { color: 'var(--text-muted)' }}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 backdrop-blur-xl"
          ...
          <div className="flex-1 max-w-md mx-4 hidden md:block">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Rechercher un dossier, un casier..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full rounded-xl py-2 pl-9 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-slate-500/40 transition-all"
                style={{
          ...
          <div className="flex items-center gap-1">
            ...
            <div ref={notifRef} className="relative">
              ...
              >
                <Bell size={20} />
                {unreadCount > 0 && (
                  <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-slate-500 flex items-center justify-center text-[9px] font-bold text-white">
              ...
              {notifOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-80 rounded-2xl shadow-2xl z-50 overflow-hidden"
                  ...
                  <div className="flex items-center justify-between px-4 py-3"
                    ...
                    <button onClick={markAllRead} className="text-[10px] text-slate-400 hover:text-slate-300 transition-colors">
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      return (
                        <div key={n.id}
                          className={`flex gap-3 px-4 py-3 transition-colors ${!n.read ? 'bg-slate-500/5' : ''}`}
                          style={{ borderBottom: '1px solid var(--border-soft)' }}
                        >
                          <div className={`p-1.5 rounded-lg border shrink-0 mt-0.5 ${notifTypeBg[n.type]}`}>
                            <NIcon size={11} className={notifTypeColor[n.type]} />
                          </div>
                          <div className="flex-1 min-w-0">
        ...
        {!online && (
          <div className="bg-amber-500/20 border-b border-amber-500/30 px-4 py-2 flex items-center justify-center gap-2 text-xs text-amber-600">
            <WifiOff size={13} />
            Mode hors ligne actif — Données mises en cache disponibles. Synchronisation dès le retour réseau.
          </div>
        )}
        ...
        <main className="flex-1 overflow-auto" style={{ background: 'var(--bg-app)' }}>
          ...
          <footer className="text-center text-[11px] py-6" style={{ color: 'var(--text-faint)' }}>
